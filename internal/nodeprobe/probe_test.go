...
func TestProbeDetectsJetsonOrinNano(t *testing.T) {
	...
	if gotLabels[chillmeta.DeviceModel] != "orin-nano" {
		t.Fatalf("DeviceModel label = %q, want orin-nano", gotLabels[chillmeta.DeviceModel])
	}
	if gotLabels[chillmeta.Accelerator] != "nvidia-jetson-orin-nano" {
		t.Fatalf("Accelerator label = %q, want nvidia-jetson-orin-nano", gotLabels[chillmeta.Accelerator])
	}
}
...
func TestProbeLeavesUnknownDeviceUnlabeled(t *testing.T) {
	...
	if facts.Annotations()[chillmeta.DeviceModelRaw] != "Generic PC" {
		t.Fatalf("raw model annotation = %q, want Generic PC", facts.Annotations()[chillmeta.DeviceModelRaw])
	}
	if facts.Annotations()[chillmeta.NodeDiscoveryReason] != chillmeta.DiscoveryReasonNoSignatureMatch {
		t.Fatalf("node discovery reason = %q, want %q", facts.Annotations()[chillmeta.NodeDiscoveryReason], chillmeta.DiscoveryReasonNoSignatureMatch)
	}
}
...
func TestProbeAnnotatesNoSourceFacts(t *testing.T) {
	...
	if annotations[chillmeta.NodeDiscoveryResult] != chillmeta.DiscoveryResultUnmatched {
		t.Fatalf("node discovery result = %q, want %q", annotations[chillmeta.NodeDiscoveryResult], chillmeta.DiscoveryResultUnmatched)
	}
	if annotations[chillmeta.NodeDiscoveryReason] != chillmeta.DiscoveryReasonNoSourceFacts {
		t.Fatalf("node discovery reason = %q, want %q", annotations[chillmeta.NodeDiscoveryReason], chillmeta.DiscoveryReasonNoSourceFacts)
	}
}
