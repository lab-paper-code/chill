...
func (f Facts) Labels() map[string]string {
	...
	addLabel(labels, chillmeta.DeviceVendor, f.Vendor)
	addLabel(labels, chillmeta.DeviceFamily, f.Family)
	addLabel(labels, chillmeta.DeviceModel, f.Model)
	addLabel(labels, chillmeta.Accelerator, f.Accelerator)
	return labels
}
...
func (f Facts) Annotations() map[string]string {
	...
	if f.RawModel != "" {
		annotations[chillmeta.DeviceModelRaw] = f.RawModel
	}
	annotations[chillmeta.DiscoverySource] = chillmeta.SourceNodeDiscovery

	switch {
	case len(labels) > 0:
		annotations[chillmeta.NodeDiscoveryResult] = chillmeta.DiscoveryResultMatched
		annotations[chillmeta.NodeDiscoveryReason] = chillmeta.DiscoveryReasonSignatureMatched
	case f.RawModel != "":
		annotations[chillmeta.NodeDiscoveryResult] = chillmeta.DiscoveryResultUnmatched
		annotations[chillmeta.NodeDiscoveryReason] = chillmeta.DiscoveryReasonNoSignatureMatch
	default:
		annotations[chillmeta.NodeDiscoveryResult] = chillmeta.DiscoveryResultUnmatched
		annotations[chillmeta.NodeDiscoveryReason] = chillmeta.DiscoveryReasonNoSourceFacts
	}
	return annotations
}
